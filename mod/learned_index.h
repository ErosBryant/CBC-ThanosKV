...
namespace adgMod {
    ...
    class LearnedIndexData {
        friend class leveldb::Version_sst;
        friend class leveldb::VersionSet_sst;
    private:
...
//        uint64_t file_size = 0;
        ...
        explicit LearnedIndexData(int allowed_seek, bool level_model) : error(file_model_error), learned(false), aborted(false), learning(false),
        ...
        bool Learn();
        bool Learned();
        //bool Learned(Version_sst* version, int v_count, int level);
        bool Learned(Version_sst* version, int v_count, FileMetaData* meta, int level);
       //  static void LevelLearn(void* arg, bool no_lock=false);
        static uint64_t FileLearn(void* arg);

        // Load all the keys in the file/level
        bool FillData(Version_sst* version, FileMetaData* meta);
    ...
    public:
        ...
        FileLearnedIndexData() : watermark(0) {};
        bool Learned(Version_sst* version, FileMetaData* meta, int level);
        bool FillData(Version_sst* version, FileMetaData* meta);
